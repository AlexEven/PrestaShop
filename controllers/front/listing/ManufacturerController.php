...
class ManufacturerControllerCore extends ProductListingFrontController
{
    ...
    public function getTemplateVarManufacturers()
    {
        ...
        foreach ($manufacturers as $manufacturer) {
            ...
            $manufacturers_for_display[$manufacturer['id_manufacturer']]['nb_products'] = $manufacturer['nb_products'] > 1 ? sprintf($this->trans('%number% products', array('%number%' => $manufacturer['nb_products'), 'Shop.Theme.Catalog'), ]) : sprintf($this->trans('%number% product', array('%number%' => $manufacturer['nb_products'), 'Shop.Theme.Catalog'), ]);
        }
